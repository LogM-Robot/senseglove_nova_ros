...
std::unique_ptr<senseglove::SenseGloveSetup> HardwareBuilder::createSenseGloveSetup()
{
	...
	std::vector<std::shared_ptr<SGCore::HapticGlove>> all_gloves = SGCore::HapticGlove::GetHapticGloves();
...
senseglove::Joint HardwareBuilder::createJoint(
	const YAML::Node& joint_config, 
...
senseglove::SenseGloveRobot HardwareBuilder::createRobot(
	const YAML::Node& robot_config, std::vector<senseglove::Joint> joints, 
	...
	int robot_index, bool is_arg_right)
{
	...
	return { glove, std::move(joints), robot_index, is_glove_right };
}
...
std::vector<senseglove::SenseGloveRobot> HardwareBuilder::createRobots(
		const YAML::Node& robots_config, std::vector<senseglove::Joint> joints,
		std::vector<std::shared_ptr<SGCore::HapticGlove>> all_gloves) const
{
	...
	{
		robots.push_back(HardwareBuilder::createRobot(robots_config, std::move(joints), glove, i, true));  // dubious fix
		i++;
	}
...
std::shared_ptr<SGCore::HapticGlove> HardwareBuilder::correct_glove(std::vector<std::shared_ptr<SGCore::HapticGlove>> gloves) const
{

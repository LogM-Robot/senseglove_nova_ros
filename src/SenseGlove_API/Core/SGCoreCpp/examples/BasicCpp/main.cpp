...
int main()
{
	...
	// It's good practice to start this process if your user has not done so yet.
	{
		...
		if (!connectionsActive) // If this process is not running yet, we can "Force-Start" SenseCom. Provided it has run on this PC at least once.
		{
			...
			if (startedSenseCom)
			{
				...
				connectionsActive = SGCore::SenseCom::ScanningActive(); //this will return false immediately after you called StartupSenseCom(). Because the program has yet to initialize.
	...
	// SenseGlove Devices are cached inside the SGCore library. The GetGloves() function returns a reference to them, not a copy.

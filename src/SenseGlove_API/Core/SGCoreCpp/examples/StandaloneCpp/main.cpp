...
#include <thread>
#include <chrono> //wait until we find SenseGloves
...
int main()
{
	...
	//---------------------------------------------------------------------------
	// SGConnect Initialization - Parts taken from the SGConnect example code.

	SGConnect::SetDebugLevel(0); //0 disables the Debug level, 1 shows you only the errors.
	...
	if (!scanActive)
	{
		...
		if (initCode > 0)
		{
			std::cout << "Successfully initialized background process: (InitCode = " << std::to_string(initCode) << ")" << std::endl;
